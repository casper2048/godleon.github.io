...
以下是稍微複雜的 Jinja2 格式，包含了 macro & loop 的應用：

```yaml
{% set flavor_name = flavor_name or "m1.medium" %}
{% set image_name = "Fedora-Cloud-Base-22.x86_64.qcow2" %}
{% set instance_max_num = 45 %}
{% set max_avg_duration = 720 %}
{% set max_seconds_per_iteration = 600 %}
...
{% macro context(users_tenants, users_per_tenant, network_start_cidr="100.1.0.0/16") -%}
context:
  users:
...
{% macro sla(max_avg_duration, max_seconds_per_iteration, failure_rate_max) -%}
sla:
...
---
NovaServers.boot_and_delete_server:
{% for i in range(5, (instance_max_num + 1)) %}
  -
    args:
      flavor:
          name: "{{ flavor_name }}"
      image:
          name: "{{ image_name }}"
      force_delete: false
    {{ runner_constant(i, i) | indent }}
    {{ context((i // 5) + 1, (i // 5) + 1) | indent }}
    {{ sla(max_avg_duration, max_seconds_per_iteration, failure_rate_max) | indent }}
{% endfor %}
```
